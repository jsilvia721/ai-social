...
describe("CSP connect-src with AWS_S3_PUBLIC_URL set (production)", () => {
  ...
  it("should include both regional and non-regional S3 hostnames when URL has no region", async () => {
    process.env.AWS_S3_PUBLIC_URL =
  ...
  it("should include both regional and non-regional S3 hostnames when URL has region", async () => {
    process.env.AWS_S3_PUBLIC_URL =
      "https://my-bucket.s3.us-east-1.amazonaws.com";
    const csp = await loadCsp();
    const connectSrc = extractDirective(csp, "connect-src");
